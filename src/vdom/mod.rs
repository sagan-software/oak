...
macro_rules! declare_elements {
    ($($x:ident)*) => ($(
        pub fn $x<Msg>() -> $crate::vdom::Element<Msg> {
            $crate::vdom::element(stringify!($x))
        }
    )*)
}
...
macro_rules! declare_text_attributes {
    ($($x:ident, $tag:expr)*) => ($(
        pub fn $x<Msg, T: ToString>(value: T) -> $crate::vdom::Attribute<Msg> {
            $crate::vdom::Attribute::Text($tag.to_owned(), value.to_string())
        }
    )*);
...
macro_rules! declare_bool_attributes {
    ($($x:ident, $tag:expr)*) => ($(
        pub fn $x<Msg>() -> $crate::vdom::Attribute<Msg> {
            $crate::vdom::Attribute::Text($tag.to_owned(), String::new())
        }
    )*);
